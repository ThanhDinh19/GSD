...
registerLicense('Ngo9BigBOggjHTQxAR8/V1JHaF1cXmhPYVFxWmFZfVhgdVVMYVxbR3VPMyBoS35RcEVmW3dfcHVVRWdVUkR2VEFe');
...
interface ExcelRoutingViewProps {
  onRefreshData?: () => void;
}
...
export default function ExcelRoutingView({ onRefreshData }: ExcelRoutingViewProps) {
  ...
  const [selectedSheet, setSelectedSheet] = useState<string>('Routing Details');
  const [tableName, setTableName] = useState<string>('routing_data');
  const [startRow, setStartRow] = useState<number>(2);
  const [endRow, setEndRow] = useState<number>(13);
  
  // Dynamic column mapping key-value items
  const [mappingItems, setMappingItems] = useState<ColumnMapItem[]>([
    { excelColumn: 'STT', dbColumn: 'stt' },
    { excelColumn: 'Mã công đoạn', dbColumn: 'op_code' },
    { excelColumn: 'Tên công đoạn', dbColumn: 'op_name' },
    { excelColumn: 'Máy sử dụng', dbColumn: 'machine' },
    { excelColumn: 'SAM', dbColumn: 'sam' },
    { excelColumn: 'SMV', dbColumn: 'smv' },
    { excelColumn: 'Tỷ lệ', dbColumn: 'rate' },
    { excelColumn: 'Độ khó', dbColumn: 'difficulty' },
    { excelColumn: 'Kỹ năng', dbColumn: 'skill' }
  ]);
  ...
  const fetchRouting = () => {
    ...
    fetch(`${API_BASE_URL}/api/routing`)
      .then(res => {
        if (!res.ok) throw new Error('Yêu cầu dữ liệu Routing thất bại');
        return res.json();
      })
      .then(data => {
      ...
      .catch(err => {
        console.error('Lỗi tải dữ liệu routing:', err);
        setLoading(false);
      });
  };

  const fetchMappingConfig = () => {
    fetch(`${API_BASE_URL}/api/mapping-config/routing_data`)
      .then(res => {
  ...
  const handleSaveData = async () => {
    ...
    try {
      ...
      // Loop from startRow to endRow (1-indexed)
      ...
      <div className="flex flex-wrap justify-between items-center gap-4">
        <div>
          <h2 className="text-base font-bold text-slate-800 flex items-center gap-2">
            <span className="text-xl">📊</span> Bảng tính Excel: Thiết lập Routing & SAM Chi Tiết
          </h2>
            ...
            <div className="py-4 overflow-y-auto space-y-5 flex-1 pr-1">
              ...
              <div className="space-y-3.5">
                <div className="grid grid-cols-2 gap-4 text-xs font-semibold text-slate-700 shrink-0">
                  <div>Cột trong file Excel <span className="text-rose-500">*</span></div>
                  <div>Cột trong bảng <span className="text-rose-500">*</span></div>
                </div>
      ...
      <div className="h-[550px] border border-slate-200 rounded overflow-hidden bg-slate-50">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-slate-500">
            ...
            <span className="text-xs font-medium">Đang nạp dữ liệu Routing từ SQL Server...</span>
          </div>
        ) : (
          <SpreadsheetComponent 
            key={key}
          ...
          >
            <SheetsDirective>
              <SheetDirective name="Routing Details">
                <RangesDirective>
                  <RangeDirective dataSource={routingData}></RangeDirective>
                </RangesDirective>
                <ColumnsDirective>
