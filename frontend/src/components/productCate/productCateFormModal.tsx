...
const emptyForm: ProductCatePayload = {
    productCode: '',
    productName: '',
    statusId: 0,
};

interface ProductCateFormModalProps {
    productCate: ProductCate | null;
    statuses: MasterStatus[];
    onClose: () => void;
    onSubmit: (payload: ProductCatePayload) => Promise<void>;
}
...
export default function ProductCateFormModal({
    ...
    onClose,
    onSubmit,
}: ProductCateFormModalProps) {
    const [form, setForm] = useState<ProductCatePayload>(emptyForm);
    const [saving, setSaving] = useState(false);

    useEffect(() => {
        if (productCate) {
            setForm({
                productCode: productCate.productCode,
                productName: productCate.productName,
                statusId: productCate.statusId,
            });
        } else {
            setForm(emptyForm);
        }
