...
const emptyForm: ProductCateGroupPayload = {
    cateGroupCode: '',
    cateGroupName: '',
    statusId: 0,
};

interface ProductCateGroupFormModalProps {
    productCateGroup: ProductCateGroup | null;
    statuses: MasterStatus[];
    onClose: () => void;
    onSubmit: (payload: ProductCateGroupPayload) => Promise<void>;
}

export default function ProductCateFormModal({
    productCateGroup,
    statuses,
    onClose,
    onSubmit,
}: ProductCateGroupFormModalProps) {
    const [form, setForm] = useState<ProductCateGroupPayload>(emptyForm);
    const [saving, setSaving] = useState(false);

    useEffect(() => {
        if (productCateGroup) {
            setForm({
                cateGroupCode: productCateGroup.cateGroupCode,
                cateGroupName: productCateGroup.cateGroupName,
                statusId: productCateGroup.statusId,
            });
        } else {
            setForm(emptyForm);
        }
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg border border-slate-200">
                <div className="px-5 py-4 border-b border-slate-100">
                    <h3 className="text-sm font-black text-slate-800 uppercase">
                        {productCateGroup ? 'Cập nhật' : 'Thêm mới'}
                    </h3>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                    <div>
                        ...
                        <input
                            value={form.cateGroupCode}
                            onChange={(e) => setForm({ ...form, cateGroupCode: e.target.value })}
                    ...
                    <div>
                        ...
                        <input
                            value={form.cateGroupName}
                            onChange={(e) => setForm({ ...form, cateGroupName: e.target.value })}
                            className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                            placeholder="..."
                            maxLength={255}
                            required
